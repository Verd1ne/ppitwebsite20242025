...
const Header = ({ menuOpen, toggleMenu }) => {
  ...
  return (
    <header className={`fixed top-0 w-full flex justify-between items-center p-5 text-xl z-[999] ${isScrolled ? 'bg-white shadow-md' : 'bg-transparent'}`}>
      {/* Logo */}
      <Link href="/">
        <img
      ...
      >
        <Link href="/committee" className={`text-2xl font-bold hover:text-[#b30000] ${menuOpen ? 'text-white' : 'text-black'}`}>
          Committee
        </Link>
        <Link href="/events" className={`text-2xl font-bold hover:text-[#b30000] ${menuOpen ? 'text-white' : 'text-black'}`}>
          Events
        </Link>
        <Link href="/faq" className={`text-2xl font-bold hover:text-[#b30000] ${menuOpen ? 'text-white' : 'text-black'}`}>
          FAQ
        </Link>
      </nav>
    </header>
